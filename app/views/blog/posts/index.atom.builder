url = if @tag.nil?
  blog_formatted_posts_path(:format => 'atom', :only_path => false)
else
  blog_posts_path(:tag => @tag, :format => 'atom', :only_path => false)
end

atom_feed(
  :url         => url,
  :root_url    => blog_posts_path(:tag => @tag, :only_path => false),
  :schema_date => '2008'
) do |feed|
